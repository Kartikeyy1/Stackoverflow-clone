  ...
  return (
    <div className="main">
        ...
        <div className='main-desc'>
            <div className='info'>
                <p>Asked<span>{new Date(questionData?.created_at).toLocaleString()}</span></p>
                    ...
                    <p>{ReactHtmlParser(questionData?.body)}</p>
                     ...
                     <div className='comments'>
                        ...
                        <div className='comment'>
                            ...
                            <p key={_qd?._id}>{_qd?.comment}{" "}<span>{_qd?.user?.displayName?_qd?.user?.displayName:String(_qd?.user?.email).split('@')[0]}</span><small>
                                {new Date(_qd?.created_at).toLocaleString()}</small></p>))
                            } 
                        </div>
                        <p onClick={()=>setShow(!show)}>Add a comment</p>
                        {
                            show && (<div className='title'><textarea value={comment} onChange={(e)=>setComment(e.target.value)}style={{margin:"5px 0px",
                            padding:"10px",
    ...
    <button type="submit" onClick={handleSubmit} style={{maxWidth:"fit-content",
marginTop:"100px",}}>Post Your answer</button>
    </div>
  );
}
